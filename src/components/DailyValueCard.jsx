  ...
  return (
    ...
    >
      <div className="w-full flex flex-row items-center justify-between mb-4">
        <h2 className='text-[#616476] font-semibold text-sm'>{category}</h2>
      ...
      <div className="w-full flex flex-row items-baseline justify-between text-center">
        <p className="text-4xl font-semibold">{formatter(value)}</p>
        <div className="flex flex-row gap-1 items-center">
          ...
          <p className={`text-xs font-semibold ${variation>0 ? 'text-[#1db489]': 'text-[#dc414c]'}`}>{variation}%</p>
        </div>
      </div>
    </div>
  );
